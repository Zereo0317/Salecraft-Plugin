...
| "SEO 做了有沒有改善" / "show me SEO improvement" / "SEO 前後比較" | `seo_verify_baseline` → `run_seo_optimize` → `seo_verify_check` → `seo_verify_report` (before/after comparison flow) |
| "改 meta title" / "換 SEO 標題" / "修改 og 圖" / manual SEO field override | `update_seo_metadata` (title / description / keywords / og_image — partial update, only provided fields change) |
...
### SEO Verification — before/after comparison（optional、免費）

Use the verification endpoints to measure the SEO improvement from optimization. This is optional but valuable for demonstrating ROI to users.

1. **Capture baseline** (before optimization):
   ```
   mcp_tool_call("landing_ai_mcp", "seo_verify_baseline", {
     "user_token": token,
     "campaign_id": campaign_id
   })
   ```

2. **Run optimization** (see `run_seo_optimize` above)

3. **Capture post-optimization snapshot**:
   ```
   mcp_tool_call("landing_ai_mcp", "seo_verify_check", {
     "user_token": token,
     "campaign_id": campaign_id
   })
   ```
...
4. **Get comparison report**:
   ```
   mcp_tool_call("landing_ai_mcp", "seo_verify_report", {
     "user_token": token,
     "campaign_id": campaign_id
   })
...
### Manual SEO metadata override（free, post-optimization）

After AI optimization, if the user wants to manually tweak specific SEO fields (e.g., "change the page title to something shorter"):

```
mcp_tool_call("landing_ai_mcp", "update_seo_metadata", {
  "user_token": token,
  "campaign_id": campaign_id,
...
All fields are optional — only the provided fields are updated; the rest remain as the AI generated them.
